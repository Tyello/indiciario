...
def test_caso_canonico_e1_distribui_suspeitas_sem_cravar_marina():
    ...
    texto_escala = str(e105.conteudo)
    assert "sem tradução nominal" in e104.objetivo_narrativo
    assert "não explica sozinha" not in texto_escala
    assert "não autoriza" not in texto_escala
    assert "não movimentar acervo sozinho" not in texto_escala
...
def test_caso_canonico_e1_falsos_caminhos_tem_limites_justos():
    ...
    assert "portaria principal" in str(e105.conteudo)
    assert "autorização administrativa" in str(e105.conteudo)
    assert "E1-05" not in str(e106.conteudo)
    assert "pacote interno de conferência" in str(e106.conteudo)
...
def test_caso_canonico_e2_tem_multiplas_empresas_e_cotacoes_sem_resposta_visual_unica():
    ...
    assert "OS, recibo e extrato" not in texto_cotacao
    assert "prazo, OS, recibo e extrato" not in texto_cotacao
...
def test_caso_canonico_e2_chat_nao_instrui_cruzamento_documental():
    blueprint = _blueprint()
    e206 = next(doc for doc in blueprint.documentos if doc.codigo == "E2-06")
    texto_chat = str(e206.conteudo)
    texto_pistas = str(e206.pistas_contidas)

    assert "confiram etiqueta, recibo e extrato" not in texto_chat.lower()
    assert "não trazem instrução explícita de cruzamento documental" in texto_pistas
    assert "recibo e extrato" not in texto_pistas.lower()
