        ...
        "documents": [
            {
                ...
                "tipo": "carta",
                "envelope": "E1",
                "source_pdf": "rendered/E1-01.pdf",
                "final_file": "01_envelope_1.pdf",
                "page_start": 1,
                "page_end": 1,
            }
        ],
...
def test_manifest_contem_arquivos_paths_relativos_e_docs_validos(tmp_path, monkeypatch):
    ...
    assert {
        "01_envelope_1.pdf",
        "02_envelope_2.pdf",
        "03_dicas_facilitador.pdf",
        "04_guia_facilitador.pdf",
        "05_guia_de_impressao.pdf",
    }.issubset(paths)
    assert all(not Path(path).is_absolute() for path in paths)
    assert all(
        entry["category"] != "player"
        for entry in manifest["files"]
        if "dicas" in entry["id"] or "gabarito" in entry["id"]
    )
...
def test_build_package_blueprint_legado_sem_contratos_nao_falha_por_graph_skipped(
    ...
    result = build_package(
        Path("examples/exemplo_blueprint.json"), tmp_path, strict=True
    )
    graph_report = json.loads(
        Path(result["graph_report_path"]).read_text(encoding="utf-8")
    )
    ...
    assert any(
        issue["code"] == "GP_006" and issue["severity"] == "warning"
...
def test_build_package_empacota_tres_envelopes_e_desloca_auxiliares(
    tmp_path, monkeypatch
):
    from generator import package_builder
    ...
    result = build_package(
        Path("examples/showcase_tecnico.json"), tmp_path, strict=True
    )
    manifest = json.loads(Path(result["manifest_path"]).read_text(encoding="utf-8"))
    ...
    assert "llm_feedback.json" not in {
        entry["file"] for entry in print_manifest["files"]
    }
    assert "playtest_report.json" not in {
        entry["file"] for entry in print_manifest["files"]
    }
    assert result["status"] == "passed"
...
def test_print_manifest_final_usa_page_count_final_do_guia(tmp_path, monkeypatch):
    ...
    manifest_guide = next(
        item for item in manifest["files"] if item["id"] == "guia_de_impressao"
    )
    print_guide = next(
        item
        for item in print_manifest["files"]
        if item["file"] == "03_guia_de_impressao.pdf"
    )
...
def test_guia_facilitador_no_manifest_print_manifest_e_qa(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _blueprint, _output_dir, strict=True: {},
    )
    monkeypatch.setattr(package_builder, "render_print_guide", fake_render_print_guide)
    monkeypatch.setattr(
        package_builder, "render_facilitator_guide", fake_render_facilitator_guide
    ...
    guia = next(
        entry for entry in manifest["files"] if entry["id"] == "guia_facilitador"
    )
...
def test_build_package_passa_html_debug_dir_sem_incluir_no_print_manifest(
    tmp_path, monkeypatch
):
    from generator import package_builder

    def fake_renderizar_caso(
        _blueprint_path, output_dir, strict=True, html_debug_dir=None
    ):
        assert html_debug_dir is not None
        html_debug_dir.mkdir(parents=True, exist_ok=True)
        (html_debug_dir / "E1-01.html").write_text(
            "<html>debug narrativo</html>", encoding="utf-8"
        )
    ...
    monkeypatch.setattr(
        package_builder,
        "build_visual_documents",
        lambda _blueprint, output_dir, strict=True: {},
    )
    monkeypatch.setattr(package_builder, "render_print_guide", fake_render_print_guide)
    monkeypatch.setattr(
        package_builder, "render_facilitator_guide", fake_render_facilitator_guide
    ...
    assert all(
        "html_debug" not in json.dumps(entry, ensure_ascii=False)
        for entry in print_manifest["files"]
    )
