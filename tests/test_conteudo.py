...
def blueprint_minimo(documentos: list[dict]) -> Blueprint:
    ...
    base = {
        ...
        "cadeia_causal": ["1", "2", "3"],
        "personagens": [
            {"id": "09", "nome": "A", "funcao": "f", "papel": "executor",
             "suspeita_aparente": "s", "verdade": "v", "documento_ancoragem": []},
            {"id": "14", "nome": "B", "funcao": "f", "papel": "red_herring",
             "suspeita_aparente": "s", "verdade": "v", "documento_ancoragem": []},
            {"id": "22", "nome": "C", "funcao": "f", "papel": "narrador",
             "suspeita_aparente": "s", "verdade": "v", "documento_ancoragem": []},
            {"id": "31", "nome": "D", "funcao": "f", "papel": "testemunha",
             "suspeita_aparente": "s", "verdade": "v", "documento_ancoragem": []},
        ],
        "linha_tempo_real": [
            {"data_hora": "10h", "evento": "e1", "documento_prova": documentos[0]["codigo"]},
            {"data_hora": "11h", "evento": "e2", "documento_prova": documentos[0]["codigo"]},
            {"data_hora": "12h", "evento": "e3", "documento_prova": documentos[0]["codigo"]},
        ],
        "pilares_validacao": [
            {"nome": "p1", "documento_principal": documentos[0]["codigo"],
             "confirmacao": documentos[1]["codigo"] if len(documentos) > 1 else documentos[0]["codigo"],
             "personagem_id": "09"},
            {"nome": "p2", "documento_principal": documentos[0]["codigo"],
             "confirmacao": documentos[0]["codigo"], "personagem_id": "09"},
            {"nome": "p3", "documento_principal": documentos[0]["codigo"],
             "confirmacao": documentos[0]["codigo"], "personagem_id": "09"},
            {"nome": "p4", "documento_principal": documentos[0]["codigo"],
             "confirmacao": documentos[0]["codigo"], "personagem_id": "09"},
        ],
        ...
        "documentos": documentos,
        "matriz_pistas": [
            {"descricao": "p", "documento": documentos[0]["codigo"],
             "o_que_sugere": "s", "o_que_prova": "p",
             "confirmacao": documentos[0]["codigo"],
             "risco_ambiguidade": "baixo", "emocao_esperada": "surpresa"},
            {"descricao": "p2", "documento": documentos[0]["codigo"],
             "o_que_sugere": "s", "o_que_prova": "p",
             "confirmacao": documentos[0]["codigo"],
             "risco_ambiguidade": "baixo", "emocao_esperada": "surpresa"},
            {"descricao": "p3", "documento": documentos[0]["codigo"],
             "o_que_sugere": "s", "o_que_prova": "p",
             "confirmacao": documentos[0]["codigo"],
             "risco_ambiguidade": "baixo", "emocao_esperada": "surpresa"},
        ],
        "red_herrings": [
            {"personagem_id": "14", "motivo_aparente": "m",
             "como_descartar": "c", "documento_descarte": documentos[0]["codigo"],
             "categoria": "motivo_sem_oportunidade"},
            {"personagem_id": "31", "motivo_aparente": "m",
             "como_descartar": "c", "documento_descarte": documentos[0]["codigo"],
             "categoria": "motivo_sem_oportunidade"},
        ],
        "dicas": [
            {"numero": i, "intensidade": "leve", "envelope": "E1",
             "condicao_uso": "c", "texto": "t", "o_que_desbloqueia": "d"}
            for i in range(1, 8)
        ],
        "versao": "0.1",
    }
...
class TestConteudoAusente:
    def test_conteudo_vazio_gera_critico(self):
        docs = [doc_base("E1-01", "email_narrador", {}),
...
class TestChavesObrigatorias:
    def test_email_completo_sem_erro_cont(self):
        docs = [doc_base("E1-01", "email_narrador", CONTEUDO_EMAIL_VALIDO),
                doc_base("E1-02", "email_narrador", CONTEUDO_EMAIL_VALIDO)]
        bp = blueprint_minimo(docs)
        r = BlueprintValidator(bp).validar()
    ...
    def test_email_faltando_assunto(self):
        ...
        docs = [doc_base("E1-01", "email_narrador", conteudo),
                doc_base("E1-02", "email_narrador", CONTEUDO_EMAIL_VALIDO)]
        bp = blueprint_minimo(docs)
        r = BlueprintValidator(bp).validar()
        assert any(e.codigo == "CONT_003" and "E1-01" in (e.documento or "")
                   for e in r.criticos)

    def test_boletim_completo_sem_erro_cont(self):
        docs = [doc_base("E1-01", "boletim", CONTEUDO_BOLETIM_VALIDO),
    ...
    def test_log_completo_sem_erro_cont(self):
        docs = [doc_base("E1-01", "log_acesso", CONTEUDO_LOG_VALIDO),
...
class TestListasObrigatorias:
    def test_registros_vazio_gera_critico(self):
        ...
        docs = [doc_base("E1-01", "log_acesso", conteudo),
                doc_base("E1-02", "log_acesso", CONTEUDO_LOG_VALIDO)]
        bp = blueprint_minimo(docs)
        r = BlueprintValidator(bp).validar()
    ...
    def test_mensagens_vazio_gera_critico(self):
        conteudo = {
            "HORA_TELA": "16:57", "CONTADOR_NAOVISTOS": "5",
            "NOME_GRUPO": "Grupo", "MEMBROS_LISTA": "A, B",
            "DATA_CONVERSA": "01 mar", "MENSAGENS": [],
        }
        docs = [doc_base("E1-01", "chat", conteudo),
                doc_base("E1-02", "email_narrador", CONTEUDO_EMAIL_VALIDO)]
        bp = blueprint_minimo(docs)
        r = BlueprintValidator(bp).validar()
        assert any(e.codigo == "CONT_004" for e in r.criticos)
...
class TestTipoSemSchema:
    def test_tipo_outro_gera_aviso_nao_critico(self):
        ...
        docs = [doc_base("E1-01", "outro", conteudo),
                doc_base("E1-02", "email_narrador", CONTEUDO_EMAIL_VALIDO)]
        bp = blueprint_minimo(docs)
        r = BlueprintValidator(bp).validar()
