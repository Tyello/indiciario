...
def assert_retrospective_examples(root: Path = RETROSPECTIVE_ROOT) -> None:
    ...
    assert len(ledgers) >= 2, "at least two retrospective ledgers are required"
    assert len(ledgers) <= 4, "ISSUE-10 should migrate only 2 to 4 retrospective ledgers"
    ...
    for ledger in ledgers:
        ...
        for session in sessions:
            ...
            assert not (FORBIDDEN_PARTICIPANT_NAMES & set(participant_blob.replace("-", " ").split()))
...
def test_guard_fails_when_no_ledger_is_found(tmp_path: Path) -> None:
    ...
    with pytest.raises(AssertionError, match="at least two retrospective ledgers"):
...
def test_guard_fails_when_source_document_is_missing(tmp_path: Path) -> None:
    ...
    data["ledgers"][0]["source_documents"] = ["docs/does-not-exist.md"]
