...
Suite de testes: `pytest tests/ -q` reporta 1511 testes coletados (checado em 2026-07-09 via `pytest tests/ --collect-only -q`; contagem cresce a cada issue — recontar antes de citar em PR).
...
- sem `judge_provider` injetado, `pipeline_runner._run_gate` continua em `gate_mode="stub"` (decisão fabricada, sem relação com desempenho real do solver) — modo `"judged"` exige provider real (ISSUE-34+);
