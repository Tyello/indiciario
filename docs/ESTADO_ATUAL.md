...
ISSUE-33.10 (diagnóstico da calibração ISSUE-33.9) concluída — "Uma Noite Sem Flores" saiu classificada "injusto" no meter porque a observabilidade de `solvability_meter.py` é insuficiente. Os 5 diagnósticos estruturalmente possíveis (H-Ja: judge semântica; H-Jb: judge evidence; H-Sa: solver divergência; H-Ra: runs incompletas; H-Ma: ambiguidade) ficaram **indeterminados** por falta de dados (solver output JSON, judge verdict JSON, run error logs nunca persistem por run). A causa raiz é a camada de instrumentação, não solver/judge/design. Relatório completo: `docs/DIAGNOSTICO_CALIBRACAO_33.10.md`. Portão de ISSUE-30.11 permanece fechado — aguardando re-execução de calibração com observabilidade melhorada (ISSUE-A proposta no relatório). Série Provider (ISSUE-33.x) documentada até 33.10; ISSUE-34 (LLM Reviewers Adapter) não iniciada.
